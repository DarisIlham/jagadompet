...
const TransactionDetailPage = () => {
  ...
  return (
    <div className="w-full min-h-screen bg-[#FA2F34] flex flex-col relative overflow-hidden">
      ...
      {/* Content Bottom */}
      <div className={`absolute w-full bg-white rounded-t-[32px] mt-[250px] flex flex-col items-center transition-all duration-300 ${isKeyboardOpen ? 'h-[562px]' : 'h-[562px]'}`}>
        
      
        <div onClick={handleCategoryClick} value={category}
              onChange={handleCategoryChange} className="flex w-[343px]  h-[74px] border-[1px] text-[#91919F] border-[#F1F1FA] rounded-[16px] mt-[23px] items-center cursor-pointer">
          <div className="flex ml-[14px] font-[400]">
            {selectedCategory ? (
              <div className={`flex items-center w-[343px]  rounded-[16px] border-transparent h-[74px] mx-[-16px] ${selectedCategoryBg}`} value={category} >
                <img src={selectedCategoryImage} alt={selectedCategory} className='w-[40px] h-[40px] ml-[16px]' />
                <span className="text-[#292B2D] font-medium text-[16px]  ml-[8px]">{selectedCategory}</span>
              </div>
            ) : (
              'Category'
            )}
          </div>
          <div className="flex w-[32px] h-[32px] ml-auto mr-[14px] items-center justify-center"><img src={downgray} alt="" /></div>
        </div>

        <div className="flex w-[343px]  h-[74px] border-[1px]   border-[#F1F1FA] rounded-[16px] mt-[16px] items-center">
          <div className="flex ml-[14px] font-[400] text-[#0D0E0F]"><input 
          placeholder='Description' 
          className='text-[#91919F]/200 border-none outline-none '/></div>
        </div>
        
        {/* Date Section with Calendar */}
        <div onClick={handleCalendarClick} className={`w-[343px] flex h-[74px] border-[1px] text-[#91919F] ${isKeyboardOpen ? 'border-transparent' : 'border-[#F1F1FA]'} rounded-[16px] mt-[16px] items-center`}>
          <div value={date}
              onChange={handleDateChange} className="flex ml-[14px] items-center font-[400]">
            {selectedDate ? (
